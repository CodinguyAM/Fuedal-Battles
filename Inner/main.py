...
#mf, mt, partway, move, board, playerText, winner, bunchobuttons, truecolors, infos, pieceInfoText, notation, moves, root
...
class FBApp(App):
    def build(self):
        ...
        self.board = [["|", "|", "|", "|", "|", "|", "|", "|", "|", "|", "|"],
                      ...
                      ["|", "|", "|", "|", "|", "|", "|", "|", "|", "|", "|"]]
        self.winner = 3
        self.infos = {".":"A board square", "|":"A border", "d":"A duke. A duke can only ever move to a square adjacent to it. Normally, this is further restricted to only the two squares diagonally in front of it, but the duke can take the power of any piece adjacent to it. The order of precedence for such power taking is king, countess, count. Note that lordly, flagly, boardsquarely, and borderly powers are excluded as they would freeze a duke. Dukes are worth 2 points.", "l":"A lord. The lord moves by jumping over pieces, it cannot cross an empty square. It also cannot move to squares directly adjacent to it. It is worth 5 points.", "k":"A king. It can move horizontally, vertically, and diagonally, but cannot jump over other pieces. It is worth 15 points.", "b":"A countess. A countess can move diagonally, but cannot jump over pieces. However, it can move exactly two spaces horizontally xor vertically, regardless of whether there is a piece on the skipped square or not. It is worth 7 points.", "c":"A count. A count can move horizontally and vertically, but cannot jump over pieces. However, it can move exactly two spaces diagonally, regardless of whether there is a piece on the skipped square or not. It is worth 8 points.", "f":"A flag. It cannot move. The objective is to protect this piece from capture. The ||(pro. fort) symbol means threat to flag, <|(pro. backflag) means threat ignored, and |>(flag)  means flag captured."}
        self.bob = []
        self.truecolors = []
        for y in range(len(self.board)):
            for x in range(len(self.board[y])):
                b = Button(text=self.board[y][x],
                           font_size = '20sp',
                           background_color = {True:(1, 0.753, 0.796), False:(0.502, 0.376, 0.302)}[abs(x - y) % 2 == 0],
                           color = {True:(0, 0, 1), False:(0, 1, 0)}[(lambda G:G==G.lower())(self.board[y][x])],
                           )
                b.bind(on_press = lambda Cyfg, x=x, y=y: self.wc(x, y))
                self.bob.append(b)
                self.truecolors.append({True:(1, 0.753, 0.796), False:(0.502, 0.376, 0.302)}[abs(x - y) % 2 == 0])
    ...
    def wc(self, bx, by):
        if self.winner == 3:
            if self.partway == 0:
                for n in range(187):
                    #print(len(self.bob), len(self.truecolors), n)
                    self.bob[n].background_color = self.truecolors[n]
                    #print('bgy')
                    #print(performMove((by, bx), (n // 11, n %11), self.board, self.move), self.board[by][bx], by, bx)
                    if type(performMove((by, bx), (n // 11, n % 11), self.board, self.move)) != type(''):
                        #print('gy')
                        self.bob[n].background_color = (1, 1, 1, 1)
                self.mf = (by, bx)
                self.partway = 1
            if self.partway == 1 and self.mf != (by, bx):
                for n in range(187):
                    self.bob[n].background_color = self.truecolors[n]
                nboard = performMove(self.mf, (by, bx), self.board, self.move)
                if type(nboard) != type(''):
                    self.board = nboard
                    for n in range(187):
                        self.bob[n].text = self.board[n//11][n%11]
                        self.bob[n].color = {True:(0, 0, 1), False:(0, 1, 0)}[(lambda G:G==G.lower())(self.board[n//11][n%11])]
                        self.bob[n].background_color = {True:(1, 0.753, 0.796), False:(0.502, 0.376, 0.302)}[abs(n//11 - n % 11) % 2 == 0]
                    ...
                    if nboard[-25:] == "is taking their own piece":
                      ...
                      for n in range(187):
                        sx = n % 11
                        sy = n // 11
                        if type(performMove(self.mf, (sy, sx), self.board, self.move)) != type(""):
                          self.bob[n].background_color = (1, 1, 1, 1)
                      partway = 1
...
def manf():
  ...
  main2()
def man():
    print('LOADING FEUDAL BATTLES APP!')
    YAY()
